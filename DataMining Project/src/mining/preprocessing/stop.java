...
public class stop{
  ...
  public ArrayList<String> removeStopWords(ArrayList<String> arrayList){
  // check words for being a stopword
  	ArrayList<String> withoutStopWords = new ArrayList<String>();
  	
  if (arrayList.size() > 0) {
    //System.out.println("\nChecking for stopwords:");
    for (int i = 0; i < arrayList.size(); i++) {
  	  String current = arrayList.get(i).toString();
      //System.out.println((i+1) + ". " + current + ": " + stop.is(current));
      
      if (!stop.is(current)) {
      	withoutStopWords.add(current);
      }
    }
  }
